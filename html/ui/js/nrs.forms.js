...
var NRS = (function (NRS, $, undefined) {
    ...
    NRS.submitForm = function ($modal, $btn, calledFromPage) {
        if (!$btn) {
        ...
        $form.find(":input").each(function () {
            if ($(this).is(":invalid")) {
                var error = "";
                var name = String($(this).attr("name")).replace("FCH", "").replace("NQT", "").capitalize();
                var value = $(this).val();
        ...
        if (data.recipient) {
            ...
            if (/^\d+$/.test(data.recipient)) {
                ...
                NRS.unlockForm($modal, $btn);
                return;
            } else if (!/^FCH\-[A-Z0-9]+\-[A-Z0-9]+\-[A-Z0-9]+\-[A-Z0-9]+/i.test(data.recipient)) {
                var convertedAccountId = $modal.find("input[name=converted_account_id]").val();
                if (!convertedAccountId || (!/^\d+$/.test(convertedAccountId) && !/^FCH\-[A-Z0-9]+\-[A-Z0-9]+\-[A-Z0-9]+\-[A-Z0-9]+/i.test(convertedAccountId))) {
        ...
        if (!NRS.showedFormWarning) {
            ...
            if (!NRS.controlAmountWarning(data, formErrorFunction, $modal, $btn, $form)) {
                isOkay = false;
            }

            if (!NRS.controlFeeWarning(data, formErrorFunction, $modal, $btn, $form)) {
                isOkay = false;
            }
    ...
    NRS.controlAmountWarning = function (data, formErrorFunction, $modal, $btn, $form) {
        if ("amountFCH" in data && NRS.settings["amountWarning"] && NRS.settings["amountWarning"] !== "0") {
            try {
                var amountNQT = NRS.convertToNQT(data.amountFCH);
            } catch (err) {
            ...
            if (new BigInteger(amountNQT).compareTo(new BigInteger(NRS.settings["amountWarning"])) > 0) {
                NRS.showedFormWarning = true;
                $form.find(".error_message").html($.t("error_max_amount_warning")).show();

                $(".form-warning").show();
                $(".form-warning .amount-warning").show();

                $(".current-amount").text(data.amountFCH);
                $(".warning-amount").text(NRS.convertToFCH(NRS.settings["amountWarning"]));
    ...
    NRS.controlFeeWarning = function (data, formErrorFunction, $modal, $btn, $form) {
        if ("feeFCH" in data && NRS.settings["feeWarning"] && NRS.settings["feeWarning"] !== "0") {
            try {
                var feeNQT = NRS.convertToNQT(data.feeFCH);
            } catch (err) {
            ...
            if (new BigInteger(feeNQT).compareTo(new BigInteger(NRS.settings["feeWarning"])) > 0) {
                NRS.showedFormWarning = true;
                $form.find(".error_message").html($.t("error_max_fee_warning")).show();

                $(".form-warning").show();
                $(".form-warning .fee-warning").show();

                $(".current-fee").text(data.feeFCH);
                $(".warning-fee").text(NRS.convertToFCH(NRS.settings["feeWarning"]));
