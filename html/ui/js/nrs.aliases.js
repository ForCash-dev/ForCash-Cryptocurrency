...
var NRS = (function(NRS, $, undefined) {
		...
		NRS.sendRequest("getUnconfirmedTransactions", {
			...
			if (response.unconfirmedTransactions && response.unconfirmedTransactions.length) {
				for (var i = 0; i < response.unconfirmedTransactions.length; i++) {
					var unconfirmedTransaction = response.unconfirmedTransactions[i];
					 ...
					 if (unconfirmedTransaction.attachment["version.AliasAssignment"] && unconfirmedTransaction.attachment["version.AliasAssignment"] === 1) {
                        aliases.push({
            ...
            NRS.sendRequest("getAliases+", {
                ...
                if (response.aliases && response.aliases.length) {
                    ...
                    aliasCount = response.aliases.length;

                    for (var i = 0; i < response.aliases.length; i++) {
                        var alias = response.aliases[i];
                        aliases.push(alias);
                    }
                }
                ...
                for (var i = 0; i < aliases.length; i++) {
                    ...
                    if (alias.status != "/") {
                        alias.status = "<span style='text-align: center' class='label label-info'>" + alias.status + "</span>";
                    }
                ...
                if (aliases.length) {
                    ...
                    $("#alias_count").html(aliasCount).removeClass("loading_dots");
                } else {
	...
	NRS.forms.sellAlias = function($modal) {
		...
		if (data.modal == "cancel_alias_sale") {
			...
			if (data.recipient == NRS.genesisRS) {
				if (!data.priceFCH || data.priceFCH == "0") {
					return {
						"error": $.t("error_not_specified", {
		...
		NRS.sendRequest("getAlias", {
			...
			if (response.errorCode) {
				...
				if (!("priceNQT" in response)) {
					...
					$modal.find("input[name=amountFCH]").val(NRS.convertToFCH(response.priceNQT)).prop("readonly", true);
				}
			}
		}, false);
	});
	...
	NRS.forms.buyAliasError = function() {
		$("#buy_alias_modal").find("input[name=priceFCH]").prop("readonly", false);
	}
	...
	function setAliasType(type, uri) {
		...
		if (type == "uri") {
			...
			$("#register_alias_uri").val("").mask("FCH-****-****-****-*****");
			...
			if (uri) {
				...
				} else if (!/^FCH\-[A-Z0-9]{4}\-[A-Z0-9]{4}\-[A-Z0-9]{4}\-[A-Z0-9]{5}/i.test(uri)) {
					uri = NRS.accountRS;
				}
					...
					if ("priceNQT" in response) {
						if (response.buyer == NRS.account) {
							message = $.t("alias_sale_direct_offer", {
								"price": NRS.formatAmount(response.priceNQT)
