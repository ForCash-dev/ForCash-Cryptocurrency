...
var NRS = (function(NRS, $, undefined) {
	...
	function displayMessageSidebar(callback) {
		...
		for (var i = 0; i < sortedMessages.length; i++) {
			...
			rows += "<a style='border-bottom: 0 !important;' href='#' class='list-group-item' data-account='" + NRS.getAccountFormatted(sortedMessage, "user") + "' data-account-id='" + NRS.getAccountFormatted(sortedMessage.user) + "'" + extra + "><h4 class='list-group-item-heading'>" + NRS.getAccountTitle(sortedMessage, "user") + "</h4><p class='list-group-item-text' style=\"color: #999;\">" + NRS.formatTimestamp(sortedMessage.timestamp) + "</p></a><div style='margin-left: 20px; border-bottom: 1px #ddd solid'></div>";
		}
			...
			for (var i = 0; i < messages.length; i++) {
				...
				if (decoded !== false) {
					...
					if (extra == "to_decrypt") {
						...
						if (type == "payment") {
							decoded = "<strong>+" + NRS.formatAmount(messages[i].amountNQT) + " FCH</strong><br />" + decoded;
						}
	...
	NRS.forms.sendMessageComplete = function(response, data) {
        ...
        NRS.contacts.forEach(function (contact) {
			if (contact.address === data.recipient) {
				addContactSuffix = "<a href='#' data-account='" + contact.address + "' data-toggle='modal' data-target='#add_contact_modal' style='text-decoration:underline'>" + $.t("add_recipient_to_contacts_q") + "</a>";
			}
		});
		...
		if (!(data["_extra"] && data["_extra"].convertedAccount)) {
			$.growl($.t("success_message_sent") + addContactSuffix, {
				"type": "success",
                "offset": 140
			});
		} else {
		...
		if (NRS.currentPage == "messages") {
			var date = new Date(Date.UTC(2019, 0, 1, 0, 0, 0, 0)).getTime();
