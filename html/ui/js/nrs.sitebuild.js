...
var NRS = (function(NRS, $, undefined) {
    ...
    NRS.loadSubheaderrHTML = function(path, options) {
        jQuery.ajaxSetup({ async: false });
        $.get(path, '', function (data) { $("body").prepend(data); });
        jQuery.ajaxSetup({ async: true });
    }
    ...
    NRS.loadModalHTMLTemplates = function(options) {
        ...
        $.get("html/modals/templates.html", '', function (data) {
            ...
            $('div[data-replace-with-html-template="recipient_modal_template"]').each(function (i) {
                var name = $(this).closest('.page').attr('id').replace('_page', '');
                var context = { name: name };
                $(this).replaceWith(template(context));
            });
            ...
            $('div[data-replace-with-html-template="add_message_modal_template"]').each(function (i) {
                var name = $(this).closest('.page').attr('id').replace('_page', '');
                var context = { name: name };
                $(this).replaceWith(template(context));
            });
            ...
            $('div[data-replace-with-html-template="secret_phrase_modal_template"]').each(function (i) {
                var name = $(this).closest('.page').attr('id').replace('_page', '');
                var context = { name: name };
                $(this).replaceWith(template(context));
            });
            ...
            $('div[data-replace-with-html-template="advanced_modal_template"]').each(function (i) {
                var name = $(this).closest('.page').attr('id').replace('_page', '');
                var context = { name: name };
                $(this).replaceWith(template(context));
            });
