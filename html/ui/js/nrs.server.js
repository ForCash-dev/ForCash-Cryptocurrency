...
var NRS = (function(NRS, $, undefined) {
	...
	NRS.sendRequest = function(requestType, data, callback, async) {
		...
		//convert NXT to NQT...
		try {
			var fchFields = ["feeFCH", "amountFCH", "priceFCH", "refundFCH", "discountFCH"];

			for (var i = 0; i < fchFields.length; i++) {
				var fchField = fchFields[i];
				var field = fchField.replace("NXT", "");

                field = fchField.replace("FCH", "");

				if (fchField in data) {
					data[field + "NQT"] = NRS.convertToNQT(data[fchField]);
					delete data[fchField];
				}
			}
		} catch (err) {
			if (callback) {
	...
	NRS.processAjaxRequest = function(requestType, data, callback, async) {
		...
		var url = NRS.server + "/fch?requestType=" + requestType;
	...
	NRS.verifyAndSignTransactionBytes = function(transactionBytes, signature, requestType, data) {
		...
		if (!("recipient" in data)) {
			//recipient == genesis
			data.recipient = "-9085696716448368263";
			data.recipientRS = "FCH-LYDT-Z7VL-KKR6-AYAU5";
		}
		...
		if (transaction.recipient !== data.recipient) {
			if (data.recipient == "-9085696716448368263" && transaction.recipient == "0") {
				//ok
			} else {
				return false;
			}
	...
	NRS.broadcastTransactionBytes = function(transactionData, callback, originalResponse, originalData) {
		$.ajax({
			url: NRS.server + "/fch?requestType=broadcastTransaction",
			crossDomain: true,
			dataType: "json",
			type: "POST",
			timeout: 30000,
