...
var NRS = (function(NRS, $, undefined) {
	...
	NRS.calculateOrderPricePerWholeQNT = function(price, decimals, returnAsObject) {
		...
		return NRS.convertToFCH(price.multiply(new BigInteger("" + Math.pow(10, decimals))), returnAsObject);
	}
	...
	NRS.calculateOrderTotal = function(quantityQNT, priceNQT) {
		...
		return NRS.convertToFCH(quantityQNT.multiply(priceNQT));
	}
	...
	NRS.convertToFCH = function(amount, returnAsObject) {
		var negative = "";
		var afterComma = "";
	...
	NRS.formatAmount = function(amount, round, no_escaping) {
		...
		if (typeof amount == "object") {
			var params = NRS.convertToFCH(amount, true);
	...
	NRS.formatTimestamp = function(timestamp, date_only) {
		if (typeof timestamp === "object") {
			...
			var date = new Date(Date.UTC(2019, 0, 1, 0, 0, 0, 0) + timestamp * 1000);
		}
		...
		if (!isNaN(date) && typeof(date.getFullYear) == 'function') {
			...
			if (!date_only) {
				...
				if (NRS.settings["hourFormat"] === "12h") {
					hours = hours % 12;
				}
				if (hours < 10) {
					hours = "0" + hours;
	...
	NRS.getAccountTitle = function(object, acc) {
		...
		NRS.contacts.forEach(function (contact) {
			if (contact.address === formattedAcc) {
	...
	NRS.createInfoTable = function(data, fixed) {
		...
		for (var key in data) {
			...
			if (/_formatted_html$/i.test(key)) {
				...
				value = NRS.formatAmount(new BigInteger(String(value))) + " FCH";
