...
final class JSONData {
    static JSONObject deleteContact(boolean successfull) {
        JSONObject json = new JSONObject();
        json.put(ContactDb.SUCCESSFULL, successfull);

        return json;
    }

    static JSONObject saveContact(int status) {
        JSONObject json = new JSONObject();
        json.put(ContactDb.STATUS, status);

        return json;
    }

    static JSONObject updateContact(int status) {
        JSONObject json = new JSONObject();
        json.put(ContactDb.STATUS, status);
    ...
    static JSONObject getSettings(Map<String, String> settings) {
        JSONObject json = new JSONObject();

        for (Map.Entry<String, String> setting: settings.entrySet()) {
            json.put(setting.getKey(), setting.getValue());
        }

        return json;
    }

    static JSONObject getSetting(String value) {
        JSONObject json = new JSONObject();

        if (value != null) {
            json.put(SettingsDb.VALUE, value);
        }

        return json;
    }

    static JSONObject updateSettings(int status) {
        JSONObject json = new JSONObject();
        json.put(ContactDb.STATUS, status);
    ...
    static JSONObject getContact(List<String> contact) {
        JSONObject json = new JSONObject();
        ...
        json.put(ContactDb.ADDRESS, contact.get(0));
        json.put(ContactDb.NICKNAME, contact.get(1));
        json.put(ContactDb.EMAIL, contact.get(2));
        json.put(ContactDb.DESCRIPTION, contact.get(3));
