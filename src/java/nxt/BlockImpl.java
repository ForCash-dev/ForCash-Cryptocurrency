...
final class BlockImpl implements Block {
    ...
    void apply() throws BlockchainProcessor.TransactionNotAcceptedException {
        long splitForgeFee = FixedAccount.splitForgingActive() ? totalFeeNQT / 2 : totalFeeNQT;

        Account generatorAccount = Account.addOrGetAccount(getGeneratorId());
        generatorAccount.apply(getGeneratorPublicKey(), this.height);
        generatorAccount.addToBalanceAndUnconfirmedBalanceNQT(splitForgeFee);
        generatorAccount.addToForgedBalanceNQT(splitForgeFee);

        FixedAccount.addForgedBalance(splitForgeFee);
